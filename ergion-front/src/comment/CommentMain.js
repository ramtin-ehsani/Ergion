...
function CommentMain() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `http://130.185.78.113:8000/api/course/chapter-episodes/?chapter_id=${chapter_id}`,
        config
      )
      .then((res) => {
        // console.log(res)
        res.data.map((episode) => {
          ...
          if (String(newEpisode.id) === String(episode_id)) {
            setPostId(newEpisode.post_id);
            setDescription(newEpisode.description);
            setLiked(newEpisode.liked);
            setLikes(newEpisode.likes);
            setFiles(newEpisode.files);
      ...
      <CssBaseline>
        ...
        >
          <Grid container item={true} xs={12} md={8} lg={8} spacing={4}>
            ...
            <Grid item style={{ width: "100%" }} dir="rtl">
                    ...
                    {files.length > 0 ? (
                          ...
                          <TableHead dir="rtl">
                            <TableRow dir="rtl" className="title">
                              <StyledTableCell align="center">
                                <Typography className="title">آیکون</Typography>
                              </StyledTableCell>
                              <StyledTableCell align="center">
                              ...
                              <StyledTableCell align="center">
                                <Typography className="title">حجم</Typography>
                              </StyledTableCell>
                              <StyledTableCell align="center">
                                <Typography className="title">
                                  دانلود
                          ...
                          <TableBody>
                            {files.map((tabFile) => (
                              <StyledTableRow dir="rtl" key={tabFile.id}>
                                <StyledTableCell align="center">
                                  <HandlePreviewIcon
                                    src={fileNameExtractor(tabFile.file)}
                                  />
                                </StyledTableCell>
                                <StyledTableCell align="center">
                                  <Box>{fileNameExtractor(tabFile.file)}</Box>
                                </StyledTableCell>
                                <StyledTableCell align="center">
                                  <div dir="ltr">
                                    <Box
                                ...
                                <StyledTableCell align="center">
                                  <Button
                                    variant="outlined"
                                    color="primary"
                                    onClick={() => handleDownload(tabFile.file)}
                                  >
                                    <GetAppRoundedIcon />
                                  </Button>
                                </StyledTableCell>
                      ...
                      >
                        <Typography
                          ...
                          className="title"
                        >
                          <Box fontSize={18}>فایلی وجود ندارد</Box>
                        </Typography>
                      </div>
                    )}
                  </TabPanel>
                    ...
                    {episodes.map((episode) => {
                      return (
                        <ListItem
                          ...
                          onClick={() => {
                            window.location = `/student_dashboard/chapter/${episode.chapter_id}/episode/${episode.id}`;
                          }}
                        >
                          <ListItemIcon>
                            <SchoolIcon />
